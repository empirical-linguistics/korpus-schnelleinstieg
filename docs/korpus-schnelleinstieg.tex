...
berücksichtigen. Deshalb ziehen wir ``Hits'' aus dem ``Zeilen''-Feld in
das ``Filter''-Feld und entfernen es aus dem ``Werte''-Feld, indem wir
es von da in das Fenster rechts oben ziehen (Fig. \ref{fig:countofhit}).
Jetzt können wir im Fenster links die nicht-prädikativen Daten
herausfiltern.
...
Referenz (Objekt vs.~Resultat) bekommen. Dafür ziehen wir die
entsprechenden Spalten in die Felder ``Zeilen'' und ``Spalten'' - was
wir in welches Fenster packen, ist eigentlich unwichtig, an den
Ergebnissen ändert das nichts. Wichtig ist, dass wir nur eine der beide
Spalten zusätzlich in das Fenster ``Werte'' ziehen. In
\ref{fig:excelpivot} sehen wir, wie das aussehen kann.

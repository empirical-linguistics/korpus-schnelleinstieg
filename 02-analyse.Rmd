...
if (knitr:::is_latex_output()) {
  knitr::include_graphics("fig/excellinewrap.png", dpi = 450)
} else {
  knitr::include_graphics("fig/excellinewrap.gif")
}
...
if (knitr:::is_latex_output()) {
  knitr::include_graphics("fig/calcformat.png", dpi = 450)
} else {
  knitr::include_graphics("fig/calcformat.gif")
}
...
if (knitr:::is_latex_output()) {
  knitr::include_graphics("fig/calc_bulkchange.png", dpi = 450)
} else {
  knitr::include_graphics("fig/calc_bulkchange.gif")
}
...
if (knitr:::is_latex_output()) {
  knitr::include_graphics("fig/excelpivot.png", dpi = 450)
} else {
  knitr::include_graphics("fig/excel_pivot.gif")
}
...
knitr::include_graphics("fig/excel_simplepivot.png", dpi = 400)
...
In unserem Beispiel wollen wir genau das tun: Wir wollen ja nur die prädikativ gebrauchten Instanzen von *(vor)programmiert* berücksichtigen. Deshalb ziehen wir "Hits" aus dem "Zeilen"-Feld in das "Filter"-Feld und entfernen es aus dem "Werte"-Feld, indem wir es von da in das Fenster rechts oben ziehen (Fig. \@ref(fig:countofhit)). Jetzt können wir im Fenster links die nicht-prädikativen Daten herausfiltern.

```{r countofhit, echo = FALSE, fig.cap="Entfernen einer Spalte aus dem \"Werte\"-Feld."}

knitr::include_graphics("fig/countofhit.png", dpi = 400)

```

Nun wollen wir eine tabellarische Übersicht über die Lesarten und die Referenz (Objekt vs. Resultat) bekommen. Dafür ziehen wir die entsprechenden Spalten in die Felder "Zeilen" und "Spalten" - was wir in welches Fenster packen, ist eigentlich unwichtig, an den Ergebnissen ändert das nichts. Wichtig ist, dass wir nur eine der beide Spalten zusätzlich in das Fenster "Werte" ziehen. In \@ref(fig:excelpivot) sehen wir, wie das aussehen kann. 
